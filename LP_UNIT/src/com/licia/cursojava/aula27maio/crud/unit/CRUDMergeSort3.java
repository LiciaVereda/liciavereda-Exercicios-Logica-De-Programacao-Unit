	...
	public static void main(String[] args) {
		...
		private static void mergeSort(float[] array,  int ini, int fim) {
			if(ini<fim) {
				...
				intercalar(array , ini, meio, fim);
				
			}
			
		}

		private static void intercalar(float[] array,int ini, int meio, int fim) {
			
			
				int tamEsq = meio - ini + 1; //tamanho do subvetor esquerdo
		        int tamDir = fim - meio; //tamanho do subvetor direito
		        float esq[] = new float[tamEsq]; //subvetor auxiliar esquerdo
		        float dir[] = new float[tamDir]; //subvetor auxiliar direito
