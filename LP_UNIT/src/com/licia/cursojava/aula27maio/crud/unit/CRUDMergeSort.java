package com.licia.cursojava.aula27maio.crud.unit;

import java.util.Scanner;


public class CRUDMergeSort {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 7:
				if (arrayExiste == true) {
					...
					float [] arrayOrdenado = new float[array.length];
					mergeSort(array,arrayOrdenado, 0, array.length-1);
		...
		private static void mergeSort(float[] array, float[] arrayOrdenado, int ini, int fim) {
			if(ini<fim) {
				int meio = (ini + fim) / 2;
				mergeSort(array,arrayOrdenado, ini, meio);
				mergeSort(array,arrayOrdenado, meio + 1, fim);
				intercalar(array , arrayOrdenado, ini, meio, fim);
				
			}
			
		}

		private static void intercalar(float[] array, float[] arrayOrdenado,int ini, int meio, int fim) {
			for (int k = 0; k <= fim; k++) {
				arrayOrdenado [k] = array[k];
				System.out.println(arrayOrdenado[k]);
			...
			for (int k = 0; k <= fim; k++) {
				if(direita > meio) {
					array [k] = arrayOrdenado[esquerda++];
				}
				else if(esquerda > fim) {
					array [k] = arrayOrdenado[direita++];
				}
				else if(arrayOrdenado[direita] < arrayOrdenado[esquerda]) {
					array [k] = arrayOrdenado[direita++];
				}
				else {
					array[k] = arrayOrdenado[esquerda++];
